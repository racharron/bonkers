...
pub fn indirect_seq(runner: impl Runner) {
    ...
    runner.when((Mut(c.clone()), Mut(d.clone())), {
        let counter = counter.clone();
        move |(c, d)| {
            assert_eq!(*c, 1);
            *c += 1;
    ...
    runner.when(Imm((a, b, c, d)), move |(a, b, c, d)| {
        assert_eq!(*a, 1);
        assert_eq!(*b, 2);
        assert_eq!(*c, 2);
        assert_eq!(*d, 1);
...
pub fn triangle(runner: impl Runner) {
    ...
    runner.when(Imm((top, right, left)), {
        let counter = counter.clone();
        move |(top, left, right)| {
